...
let Shaders = {}
	...
	`
    varying vec2 vUv;
	vec3 color = vec3(0.5, 0.5, 1.0);         //local variable red
	    void main() {
		    gl_FragColor = vec4( vec3(color),  vUv);    //alpha is 0.75
	    }`
    };
    ...
    Shaders.TextureShader1 = {

        name: 'TextureShader1', //for debugging
        ...
        void main() {
            vec3 pos = position;
            pos.x += sin( pos.y );
       	    pos.x += 0.4 * sin( time * 2.0 + pos.x ) + 0.2 * sin( time * 2.0 + pos.y );
            pos.y += sin( pos.y );
       	    pos.y += 0.4 * sin( time * 2.0 + pos.x ) + 0.2 * sin( time * 2.0 + pos.y );
            pos.z += sin( pos.y );
       	    pos.z += 0.4 * sin( time * 2.0 + pos.x ) + 0.2 * sin( time * 2.0 + pos.y );
    ...
    Shaders.TextureShader2 = {

        name: 'TextureShader2', //for debugging
...
const __shaderT1 = Shaders.TextureShader1;
const __shaderT2 = Shaders.TextureShader2;
...
function createGeometry() {
    ...
    textureShaderMaterial2 = new THREE.ShaderMaterial(
        {
    ...
    let tetra = new THREE.Mesh(new THREE.TetrahedronBufferGeometry(6, 10), basicShaderMaterial2);
    tetra.position.set(-15, 10, 15);
    scene.add(tetra);

    let ball = new THREE.Mesh( new THREE.SphereBufferGeometry( 3, 128, 128 ), textureShaderMaterial2 );
    ball.position.set(-15,10,-15);
...
function render() {
    ...
    __shaderT1.uniforms.time.value = clock.getElapsedTime();
    __shaderT2.uniforms.time.value = clock.getElapsedTime();
